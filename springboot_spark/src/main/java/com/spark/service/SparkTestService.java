...
import com.spark.receiver.CustomReceiver;
...
public class SparkTestService {
	...
	public Map<String, Object> calculateTopTen() {
		...
		String filePath = this.getClass().getResource("/static/test.txt").getPath();
		...
		JavaPairRDD<String, Integer> ones = words.mapToPair(str -> new Tuple2<>(str, 1));
		
		JavaPairRDD<String, Integer> counts = ones.reduceByKey((Integer i1, Integer i2) -> (i1 + i2));
		
		JavaPairRDD<Integer, String> temp = counts.mapToPair(tuple -> new Tuple2<>(tuple._2, tuple._1));
		
		JavaPairRDD<String, Integer> sorted = temp.sortByKey(false).mapToPair(tuple -> new Tuple2<>(tuple._2, tuple._1));
